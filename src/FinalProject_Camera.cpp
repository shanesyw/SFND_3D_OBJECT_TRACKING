...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        detectObjects((dataBuffer.end() - 1)->cameraImg, (dataBuffer.end() - 1)->boundingBoxes, confThreshold, nmsThreshold,
                      yoloBasePath, yoloClassesFile, yoloModelConfiguration, yoloModelWeights, false);  //changed last bool to false to suppress visualization

        //cout << "#2 : DETECT & CLASSIFY OBJECTS done" << endl;

        // my test code to check bounding box
        if(false)
        {
            ...
            for (auto it = (dataBuffer.end() - 1)->boundingBoxes.begin(); it != (dataBuffer.end() - 1)->boundingBoxes.end(); ++it)
            {
                ...
                label = to_string(it - (dataBuffer.end() - 1)->boundingBoxes.begin());//(*it).boxID;
                //cout << "bounding box id: " << label << endl;
        ...
        */
        ...
        else if(detectorType.compare("HARRISCORNER") == 0)
        {
        ...
        if (bFocusOnVehicle)
        {
            ...
            {
                if(keypoints[i].pt.x >= 535 && keypoints[i].pt.x <= (535+180) &&
                    keypoints[i].pt.y >= 180 && keypoints[i].pt.y <= (180+150))
                {
            ...
            //std::cout << "Points on preceding vehicle: " << numOfPoint << "" << std::endl;
            //std::cout <<  numOfPoint << "|" << std::endl;
        }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            // test code. try to see if 2D matching is done properly
            if (false)
            {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                for (auto it2 = (dataBuffer.end() - 1)->boundingBoxes.begin(); it2 != (dataBuffer.end() - 1)->boundingBoxes.end(); ++it2)
                {
                    ...
                    {
                        //std::cout << "current match partner corresponds to this BB" << endl;
                        currBB = &(*it2);
                        currBB_found = true;
                        currBB_ind = it2->boxID;
                    }
                }
                ...
                {
                    std::cout << "we have a match. Current: " << currBB_ind <<
                        " Previous: " << prevBB_ind << endl;
                }
                else
                    //std::cout << "didn't find a match!!" << endl;
                */
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    //computeTTCCamera((dataBuffer.end() - 2)->keypoints,
                    //    (dataBuffer.end() - 1)->keypoints, *currBB, currBB->kptMatches,
                    //    sensorFrameRate, ttcCamera);
                    computeTTCCamera2((dataBuffer.end() - 2)->keypoints,
                        (dataBuffer.end() - 1)->keypoints, currBB->kptMatches, 
                        sensorFrameRate, ttcCamera);
                    //// EOF STUDENT ASSIGNMENT
